...
Snake.prototype.consume = function(food) {
	...
	this.game.fireEvent ("consume", food);
...
var Renderer = function(canvasId, game) {
	...
	this.backgroundCanvas = document.getElementById(canvasId+"blur");
	this.canvas.width = game.board.width * TILE_SIZE;
	this.canvas.height = game.board.height * TILE_SIZE + 70;
	...
	var that = this;
	game.registerListener("consume", 
		function (food) {
			that.renderConsumeEffect (that.backgroundCanvas.getContext('2d'), food);
		}
	);

	//this.setupBackground(this.backgroundCanvas.getContext('2d'), game);
};
...
Renderer.prototype.renderConsumeEffect = function(ctx, food) {
	...
	var coord = food.coordinate.scale(TILE_SIZE).translate(new Vector(1,1).scale(-0.5*(rad - TILE_SIZE)));

	if (food.type.powerUps.length > 0) {
		ctx.fillRect(coord.x, coord.y, rad, rad);
	} else {
		ctx.drawCircle(coord.x, coord.y, rad).fill();
	}
	ctx.restore();
};

Renderer.prototype.setupBackground = function(ctx, game) {
	this.imgData = ctx.createImageData(this.canvas.width, this.canvas.height);
}

Renderer.prototype.clearCanvas = function(ctx, game) {
	/*ctx.save();
...
Renderer.prototype.renderBackground = function(ctx, game, deltaTime) {
	...
	for (var y=1;y<h-1;y++) {
		for (var x=1;x<w-1;x++) {
			for (var c=0;c<3;c++) {
				var i = (y*w + x) * 4;
				var val = data[i+c] + data[(y*w + x -1)*4 + c] + data[(y*w + x +1)*4 + c] + data[((y-1)*w + x)*4 + c] + data[((y+1)*w + x)*4 + c];
				val *= 0.199;
				if (val < 10) val *= 0.8;
				if (val < 5) val = 0;
				data[i+c] = val;
...
Renderer.prototype.renderBoard = function(ctx, game, deltaTime) {
	ctx.save();
...
Renderer.prototype.renderMeter = function(ctx, game, deltaTime) {
	ctx.save();
...
Renderer.prototype.updatePulse = function(hunger, elapsed, current) {
	...
	var derivative = elapsed / 1000 * 0.05;
	if (current < 0.05) {
...
Renderer.prototype.renderSnake = function(ctx, game, deltaTime) {
	ctx.save();
	var snakeSegments = game.snake.segments;
	this.snakeLight = this.updatePulse(game.hunger, deltaTime, this.snakeLight);
...
Renderer.prototype.renderFood = function(ctx, game, deltaTime) {
	ctx.save();
	var foods = game.foods;
	var pulse = (this.lastRender / 20) % 200;
	if (pulse > 100)
		pulse = 200 - pulse;
	pulse = 20 + pulse / 3;
	ctx.shadowBlur = 30;
...
Game.prototype.fireEvent = function(eventName) {
...
function startGame() {
	...
	//preventDefault will prevent scrolling on small screens
	$("body").bind({
		keydown: function(e) {
			var key = e.keyCode;
